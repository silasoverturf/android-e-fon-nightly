 ...
 */	
	public class SplitTextField extends Sprite {
		...
		public static function split(source:TextField, spitType:String="characters", container:DisplayObjectContainer=null, offset:Point=null):Array {
			...
			var segments:Array;
			var cnt:uint = 0;
			source.appendText(""); //just to force a refresh of numLines because a bug in the Flash player can cause numLines to be incorrect if a TextField's width is changed after its text is set.
			...
			for (line = 0; line < linesTotal; line++) {
				...
				for (i = 0; i < totalSegments; i++) {
					...
					x += tf.textWidth;
					if ((segments[i] != " " || format.underline) && tf.textWidth != 0) { //don't include carriage returns which have a textWidth of 0
						fields[cnt++] = tf;
					}
				}
