 ...
 */	 
	public class LiquidStage extends EventDispatcher {
		...
		public function attach(target:DisplayObject, pin:PinPoint, strict:Boolean=false, reconcile:Boolean=true, tweenDuration:Number=0, tweenVars:Object=null, roundPosition:Boolean=false):void {
			...
			LiquidData.addCacheData(this, new LiquidData(pin, target, 2, this, strict, tweenDuration, tweenVars, roundPosition));
			if (tempRetro) {
		...
		private function updateItems():void {
			...
			var item:LiquidData, p:Point, p2:Point, xDif:Number, yDif:Number, tx:Number, ty:Number;
			var revert:Array = [];
			var cnt:uint = 0;
			...
			while (--i > -1) {
				...
				//item is a DisplayObject that has been pinned
					...
					if (item.tween) {
						...
						tx = item.tween.vars.x;
						ty = item.tween.vars.y; 
						
					} else {
						tx = item.target.x + xDif;
						ty = item.target.y + yDif;
					}
		...
		/** A Sprite that starts out at the native size of the stage (baseWidth/baseHeight) and then gets scaled to fit the stage - you can use this for the <code>target</code> parameter for custom PinPoints that you place positions on the stage. **/
